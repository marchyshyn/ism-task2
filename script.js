function up () {
	...
	if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
		$(".slider-img").on("touchstart",function (event){
			event.stopPropagation();
			$('.slider-img').removeClass('pack-up');
			$('.slider-img').stop().animate({top: '0px'}, 600, 'easeOutBounce');
			$('.shadow', $(this).parent()).removeClass('shadow-scale-back');
			$('.shadow', $(this).parent()).addClass('shadow-scale');
			$('.mess').animate({opacity: 0}, 300);
		...
		$('.popup-bg, header, .close-popup').on('touchstart', function() {
